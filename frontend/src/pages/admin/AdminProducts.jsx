...
const AdminProducts = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await api.get(`/admin/products?status=${tab}&limit=50`);
  ...
  const approve = async (id) => {
    ...
    try { await api.put(`/admin/products/${id}/approve`); load(); }
  ...
  const reject = async () => {
    ...
    try {
      await api.put(`/admin/products/${rejectModal}/reject`, { reason: rejectReason || 'Does not meet requirements' });
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 20px' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--muted)' }}>
          ...
          <p>No {tab === 'pending-approval' ? 'pending' : tab} products</p>
        </div>
      ) : (
            ...
            <div key={item._id} style={{
              ...
              <div style={{ flex: 1, minWidth: 180 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                  <span style={{ fontWeight: 700, fontSize: '0.96rem', color: 'var(--text)' }}>{item.name}</span>
                  {badge(item.status)}
                </div>
                <div style={{ fontSize: '0.82rem', color: 'var(--muted)' }}>
                  by {item.vendor?.businessName} · {item.category}
                </div>
      ...
      {rejectModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0 0 14px', color: 'var(--text)', fontSize: '1rem', fontWeight: 700 }}>Reject Product</h3>
